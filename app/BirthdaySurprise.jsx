    ...
    {Array.from({ length: total }, (_, i) => (
      <div key={i} style={{
        ...
        background: i < current
          ? `linear-gradient(90deg, ${theme.pink}, ${theme.purple})`
          : theme.border,
    ...
    <div style={{
      ...
      <div style={{
        ...
        <h1 className="display" style={{
          ...
          Make someone's<br />
          <span className="gradient-text">birthday unforgettable</span>
        </h1>

        <p style={{
          fontSize: "clamp(15px, 3.5vh, 17px)",
          color: theme.muted,
          lineHeight: 1.5,
        ...
        <div style={{
          ...
          <button className="btn-primary" style={{ 
            fontSize: "clamp(15px, 3.5vh, 16px)", 
            padding: "clamp(12px, 3vh, 16px) clamp(24px, 6vh, 36px)" 
          }} onClick={() => onNavigate("create")}>
            Create a Surprise 🎉
          </button>
          <button className="btn-ghost" style={{ 
            fontSize: "clamp(14px, 3vh, 15px)", 
            padding: "clamp(10px, 2.5vh, 12px) clamp(20px, 5vh, 28px)" 
          }} onClick={() => onNavigate("view", { demo: true })}>
            See a Demo
          </button>
        </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 440, width: "100%", position: "relative", zIndex: 1, animation: "fade-up 0.5s ease forwards" }}>
        <button onClick={() => step === 1 ? onNavigate("home") : setStep(1)}
          style={{ background: "none", border: "none", color: theme.muted, cursor: "pointer", fontSize: 14, marginBottom: 24, display: "flex", alignItems: "center", gap: 6 }}>
          ← Back
        </button>

        <StepBar current={step} total={2} />

        <h2 className="display" style={{ fontSize: "clamp(24px, 6vw, 28px)", fontWeight: 700, marginBottom: 6 }}>
          {step === 1 ? "Who's this for?" : "Add the magic ✨"}
        </h2>
        <p style={{ color: theme.muted, fontSize: 14, marginBottom: 28 }}>
          {step === 1 ? "Tell us who's sending and who's receiving." : "Write a heartfelt message and add a photo."}
        </p>
          ...
          {step === 1 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              <div>
                ...
                <input className="input-field" placeholder="e.g. Hassan" value={form.sender}
              ...
              <div>
                ...
                <input className="input-field" placeholder="e.g. Sara" value={form.receiver}
          ...
          {step === 2 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              <div>
                ...
                <textarea className="input-field" placeholder={`Say something beautiful to ${form.receiver || "them"}...`}
              ...
              <div>
                ...
                <div onClick={() => fileRef.current.click()} style={{
                  ...
                  background: form.imagePreview ? "transparent" : "rgba(255,255,255,0.02)",
                  overflow: "hidden",
                }}>
                  {form.imagePreview ? (
                    ...
                    <>
                      <div style={{ fontSize: "clamp(20px, 6vw, 24px)", marginBottom: 8 }}>🖼️</div>
                      <div style={{ color: theme.muted, fontSize: "clamp(12px, 3vw, 13px)" }}>Tap to upload</div>
                    </>
                  )}
                </div>
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 440, width: "100%", position: "relative", zIndex: 1 }}>
        ...
        <div style={{
          ...
          <p style={{ color: theme.muted, fontSize: 14, marginBottom: 24 }}>Here's how it looks. Share the link when you're ready.</p>
          ...
          <div className="glass" style={{ padding: "clamp(20px, 5vw, 24px)", marginBottom: 16, textAlign: "center" }}>
            ...
            {form.imagePreview && (
              <img src={form.imagePreview} alt="preview" style={{
...
const ViewPage = ({ data }) => {
  ...
  useEffect(() => {
    ...
    if (count <= 0) {
      const timer = setTimeout(() => setPhase("gift"), 400);
      return () => clearTimeout(timer);
    }
  ...
  if (phase === "intro") return (
    <div style={{
      ...
      alignItems: "center", justifyContent: "center", padding: "clamp(24px, 8vw, 32px)",
      position: "relative", textAlign: "center",
    }}>
      <OrbBg orbs={[
        { color: theme.purple, size: 400, top: "-5%", left: "-5%", opacity: 0.15 },
        { color: theme.pink, size: 300, bottom: "5%", right: "-5%", opacity: 0.12 },
      ]} />
      <div style={{ position: "relative", zIndex: 1, animation: "scale-in 0.6s cubic-bezier(0.34,1.56,0.64,1)" }}>
        ...
        <button className="btn-primary" style={{ fontSize: "clamp(14px, 4vw, 16px)", padding: "clamp(14px, 4vw, 16px) clamp(32px, 10vw, 40px)" }}
  ...
  if (phase === "countdown") return (
    <div style={{
      ...
      <div style={{
        ...
        animation: count > 0 ? "scale-in 0.3s cubic-bezier(0.34,1.56,0.64,1)" : "",
      }}>
        <div className="display gradient-text" style={{
          fontSize: "clamp(80px, 30vw, 200px)",
          fontWeight: 700,
          lineHeight: 1,
          textAlign: "center",
        ...
        {count > 0 && (
          <p style={{ textAlign: "center", color: theme.muted, fontSize: "clamp(14px, 4vw, 16px)", marginTop: 16 }}>Get ready…</p>
        )}
      </div>
    </div>
  );
  ...
  if (phase === "gift") return (
    <div style={{
      ...
      <OrbBg orbs={[
        { color: theme.gold, size: 350, top: "30%", left: "30%", opacity: 0.12 },
      ]} />
      <div style={{ position: "relative", zIndex: 1 }}>
        <p style={{ color: theme.muted, fontSize: "clamp(13px, 4vw, 15px)", marginBottom: 20 }}>Tap the gift to open 🎁</p>
        <div onClick={handleOpenGift} style={{
          fontSize: "clamp(80px, 25vw, 140px)",
          cursor: "pointer",
          transition: "transform 0.3s",
          animation: giftOpen ? "bounce-in 0.5s forwards" : "float 2s ease-in-out infinite",
          display: "inline-block",
          userSelect: "none",
          filter: giftOpen ? "brightness(1.5) drop-shadow(0 0 30px rgba(255,209,102,0.6))" : "none",
          transform: giftOpen ? "scale(1.3) rotate(10deg)" : "",
        }}>
          {giftOpen ? "✨" : "🎁"}
        </div>
        {!giftOpen && (
          <p style={{ marginTop: 20, color: theme.muted, fontSize: "clamp(12px, 3.5vw, 14px)", animation: "pulse-glow 2s ease-in-out infinite" }}>
            tap to unwrap
          </p>
        )}
      </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: "clamp(24px, 7vw, 44px)", fontWeight: 700, lineHeight: 1.2, marginBottom: 6 }}
          className="display">
          Happy Birthday,
        </div>
        <div className="display gradient-text" style={{ fontSize: "clamp(28px, 9vw, 58px)", fontWeight: 700, marginBottom: "clamp(20px, 6vw, 28px)" }}>
          {form.receiver}! 🎂
        </div>
        ...
        {msgDone && (
          <div style={{
            ...
            <span style={{ color: theme.muted, fontSize: "clamp(13px, 3.5vw, 14px)" }}>
              with love from <strong style={{ color: theme.text }}>{form.sender}</strong>
            </span>
          </div>
        )}
